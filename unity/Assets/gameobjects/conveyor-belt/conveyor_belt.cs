...
public class conveyor_belt : Node
{

    [SerializeField] private float laneInitialPosition = -0.865f;
    [SerializeField] private float laneFinalPosition = 0.865f;
    private float speed;
    private Vector3 itemInitialPosition;

    protected override void start()
    {
        itemInitialPosition = new Vector3(laneInitialPosition, 0.05f, 0);
    ...
    {
        item.item.SetParent(transform);
        item.item.rotation = Quaternion.identity;
        item.item.localPosition = itemInitialPosition;
    }
    ...
    {
        item.item.localPosition += new Vector3(speed * Time.deltaTime, 0, 0);
